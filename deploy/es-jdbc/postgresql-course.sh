...
echo '{
	...
	"schedule" : "0 0/2 * * * ?",
        ...
        "user" : "lrs",
        "password" : "123456",
	"statefile" : "course-statefile.json",
        "sql" : [
	  {
		"statement":"SELECT id as _id, name, description, status, logo, authors, category as tag, stored, repository, owner, type FROM project WHERE type='"'"'course'"'"' AND stored > ?",
